...
Sbuild::Version - handling and comparing dpkg-style version numbers

=head1 DESCRIPTION

The Sbuild::Version module provides pure-Perl routines to compare
dpkg-style version numbers (as used in Debian packages) and also
an object oriented interface overriding perl operators
to do the right thing when you compare Sbuild::Version object between
them.
...
=item my $v = Sbuild::Version->new($version, %opts)

Create a new Sbuild::Version object corresponding to the version indicated in
...
When the Sbuild::Version object is used in a boolean evaluation (for example
...
needs to be a Sbuild::Version, the other one can be anything provided that
...
sub comparison {
    my ($a, $b, $inverted) = @_;
    if (not ref($b) or not $b->isa("Sbuild::Version")) {
        $b = Sbuild::Version->new($b);
    }
    ($a, $b) = ($b, $a) if $inverted;
...
sub version_compare($$) {
    my ($a, $b) = @_;
    my $va = Sbuild::Version->new($a, check => 1);
    defined($va) || error(_g("%s is not a valid version"), "$a");
    my $vb = Sbuild::Version->new($b, check => 1);
