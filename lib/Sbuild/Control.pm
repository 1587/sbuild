...
use Sbuild::Control::Hash;
use Sbuild::Control::Fields;

use base qw(Sbuild::Control::Hash Exporter);
...
Sbuild::Control - parse and manipulate official control-like information

=head1 DESCRIPTION

The Sbuild::Control object is a smart version of Sbuild::Control::Hash.
...
All the methods of Sbuild::Control::Hash are available. Those listed below
are either new or overridden with a different behaviour.

=over 4

=item my $c = Sbuild::Control->new(%opts)
...
sub new {
    ...
    my $self = Sbuild::Control::Hash->new();
    bless $self, $class;
...
Sbuild::Control::Fields::field_ordered_list($type).
