...
Sbuild::Deps - parse and manipulate dependencies of Debian packages

=head1 DESCRIPTION

The Sbuild::Deps module provides objects implementing various types of
dependencies.

The most important function is deps_parse(), it turns a dependency line in
a set of Sbuild::Deps::{Simple,AND,OR,Union} objects depending on the case.
...
# Sbuild::Deps::_arch_is_superset(\@p, \@q)
#
...
exported by Sbuild::Version: REL_EQ, REL_LT, REL_LE, REL_GT, REL_GT.
...
The $v_p and $v_q parameter should be Sbuild::Version objects.
...
Sbuild::Deps::AND or a Sbuild::Deps::Union. Various options can alter the
...
set to 1. By default it uses Sbuild::Arch::get_host_arch() to identify
...
If set to 1, returns a Sbuild::Deps::Union instead of a Sbuild::Deps::AND. Use
...
sub deps_parse {
    ...
    foreach my $dep_and (split(/\s*,\s*/m, $dep_line)) {
	    ...
	    my $dep_simple = Sbuild::Deps::Simple->new($dep_or);
    ...
    my $dep_and;
    if ($options{union}) {
	$dep_and = Sbuild::Deps::Union->new();
    } else {
	$dep_and = Sbuild::Deps::AND->new();
    }
    foreach my $dep (@dep_list) {
        if ($options{union} and not $dep->isa("Sbuild::Deps::Simple")) {
...
sub deps_compare {
    ...
    while ($a->isa('Sbuild::Deps::Multiple')) {
	return -1 if $a->is_empty();
	my @deps = $a->get_deps();
	$a = $deps[0];
    }
    while ($b->isa('Sbuild::Deps::Multiple')) {
	return 1 if $b->is_empty();
	my @deps = $b->get_deps();
	$b = $deps[0];
    }
...
package Sbuild::Deps::Simple;

=head1 OBJECTS - Sbuild::Deps::*

There are several kind of dependencies. A Sbuild::Deps::Simple dependency
represents a single dependency statement (it relates to one package only).
Sbuild::Deps::Multiple dependencies are built on top of this object
and combine several dependencies in a different manners. Sbuild::Deps::AND
represents the logical "AND" between dependencies while Sbuild::Deps::OR
represents the logical "OR". Sbuild::Deps::Multiple objects can contain
Sbuild::Deps::Simple object as well as other Sbuild::Deps::Multiple objects.
...
Sbuild::Deps::KnowFacts is a special object that is used while evaluating
...
information. This is true when a Sbuild::Deps::Simple object has not yet
been initialized or when a (descendant of) Sbuild::Deps::Multiple contains
an empty list of dependencies.

=item $dep->get_deps()

Return a list of sub-dependencies. For Sbuild::Deps::Simple it returns
itself.
...
architecture. A Sbuild::Deps::Simple object can be left empty after this
operation. For Sbuild::Deps::Multiple objects, the non-relevant
sub-dependencies are simply removed.

This trims off the architecture restriction list of Sbuild::Deps::Simple
objects.
...
Sbuild::Deps::KnownFacts object given as parameters.
...
object Sbuild::Deps::KnownFacts) and a list of other dependencies that we
know to be true.
...
sub parse_string {
    ...
    if (defined($3)) {
        $self->{version} = Sbuild::Version->new($3);
    }
    if (defined($4)) {
...
sub implies {
	...
	return undef unless Sbuild::Deps::_arch_is_superset($self->{arches}, $o->{arches});
	...
	return Sbuild::Deps::deps_eval_implication($self->{relation},
		$self->{version}, $o->{relation}, $o->{version});

    } elsif ($o->isa('Sbuild::Deps::AND')) {
	...
	internerr("Sbuild::Deps::Simple can't evaluate implication with a %s!",
	          ref($o));
    }
}
...
sub merge_union {
    my ($self, $o) = @_;
    return 0 if not $o->isa('Sbuild::Deps::Simple');
...
package Sbuild::Deps::Multiple;

=head2 Sbuild::Deps::Multiple

This the base class for Sbuild::Deps::{AND,OR,Union}. It contains the
...
sub sort {
    my $self = shift;
    my @res = ();
    @res = sort { Sbuild::Deps::deps_compare($a, $b) } @{$self->{list}};
    $self->{list} = [ @res ];
}
...
sub merge_union {
    internerr("The method merge_union() is only valid for Sbuild::Deps::Simple");
}

package Sbuild::Deps::AND;

=head2 Sbuild::Deps::AND
...
use base qw(Sbuild::Deps::Multiple);
...
package Sbuild::Deps::OR;

=head2 Sbuild::Deps::OR
...
package Sbuild::Deps::KnownFacts;

=head2 Sbuild::Deps::KnowFacts
...
=item my $facts = Sbuild::Deps::KnownFacts->new();
