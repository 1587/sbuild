...
Sbuild::Control::Info - parse files like debian/control
...
=item $c = Sbuild::Control::Info->new($file)

Create a new Sbuild::Control::Info object for $file. If $file is omitted, it
...
sub parse {
    my ($self, $fh, $desc) = @_;
    $self->reset();
    my $cdata = Sbuild::Control->new(type => CTRL_INFO_SRC);
    ...
    while (1) {
	$cdata = Sbuild::Control->new(type => CTRL_INFO_PKG);
...
Returns a Sbuild::Control object containing the fields concerning the
source package.
...
Returns a Sbuild::Control object containing the fields concerning the binary
...
Returns a list containing the Sbuild::Control objects for all binary packages.
...
Return a list of Sbuild::Control objects, the first one is corresponding to
