...
# This module must absolutely not use Sbuild::Control::Fields
# it's used by other modules that are required to compile
# Sbuild::Control::Fields itself (Dpkg::Vendor)
...
Sbuild::Control::Hash - parse and manipulate a block of RFC822-like fields

=head1 DESCRIPTION

The Sbuild::Control::Hash object is a hash-like representation of a set of
...
=item my $c = Sbuild::Control::Hash->new(%opts)
...
sub new {
    ...
    $$self->{'fields'} = Sbuild::Control::Hash::Tie->new($self);
...
package Sbuild::Control::Hash::Tie;
...
# Sbuild::Control::Fields::field_capitalize). It also stores the order in
...
# Sbuild::Control::Hash->new($parent)
#
...
sub TIEHASH  {
    my ($class, $parent) = @_;
    die "Parent object must be Sbuild::Control::Hash"
        if not $parent->isa("Sbuild::Control::Hash");
