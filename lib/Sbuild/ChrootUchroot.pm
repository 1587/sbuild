...
sub begin_session {
    ...
    my $uchroot_session=readpipe($self->get_conf('UCHROOT') . " -c $chroot --begin-session");
    chomp($uchroot_session);
    if ($?) {
	print STDERR "Chroot setup failed\n";
	return 0;
    }

    $self->set('Session ID', $uchroot_session);
    print STDERR "Setting up chroot $chroot (session id $uchroot_session)\n"
	if $self->get_conf('DEBUG');

    my $info = $self->get('Chroots')->get_info($uchroot_session);
	if (defined($info) &&
...
sub end_session {
	...
	if $self->get_conf('DEBUG');
    system($self->get_conf('UCHROOT') . ' -c ' . $self->get('Session ID') . ' --end-session');
    $self->set('Session ID', "");
    if ($?) {
...
sub get_command_internal {
    ...
    @cmdline = ($self->get_conf('UCHROOT'),
		'-d', $dir,
		'-c', $self->get('Session ID'),
		'--run-session',
		@{$self->get_conf('UCHROOT_OPTIONS')},
		'-u', "$user", '-p', '--');
    if (ref $command) {
